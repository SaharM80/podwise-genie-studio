...
import { Mic, FileText, Download, Loader2 } from "lucide-react";
import { Navigation } from "@/components/ui/navigation";
import { useToast } from "@/hooks/use-toast";
import { generateTranscript, generatePodcast } from "@/api";
...
export default function Generate() {
  ...
  const [result, setResult] = useState<{ transcript?: string; audioUrl?: string } | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      let data: { transcript?: string; podcast?: string } = {};

      if (outputType === "transcript") {
        data = await generateTranscript(prompt);
        setResult({ transcript: data.transcript, audioUrl: undefined });
      } else if (outputType === "audio") {
        data = await generatePodcast(prompt);
        setResult({ transcript: undefined, audioUrl: data.podcast });
      } else if (outputType === "both") {
        data = await generatePodcast(prompt);
        setResult({ transcript: data.transcript, audioUrl: data.podcast });
      }
  ...
  const handleDownloadAudio = () => {
    if (result?.audioUrl) {
      ...
      link.download = "podcast.wav"; // forces browser download
      link.click();
    }
  };
                  ...
                  <div className="space-y-6">
                    {result.transcript && (
                      <div>
                        ...
                        <Button variant="outline" size="sm" className="mt-2" onClick={handleDownloadTranscript}>
